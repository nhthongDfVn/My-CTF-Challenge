...
<link href="assets/dist/css/bootstrap.min.css" rel="stylesheet">
  ...
  <div class="checkbox mb-3">
    <label>
      Have another account? <a href="login.php">Login</a>
    </label>
    <?php
       if (isset($_POST['username'])&&!empty($_POST['username'])&&isset($_POST['password'])&&!empty($_POST['password'])){
                    $username=$_POST['username'];
                    $password=$_POST['password'];
                    if (strlen($username)<5||strlen($password)<7||strlen($username)>95){
                       echo "<div class='alert alert-danger' role='alert'>
                            <strong>Invalid length</strong>(Username >5, Password >8)</div>";
                    }
                    else{
  ...
  <p class="mt-5 mb-3 text-muted text-center">&copy; ANHTOICTF 2021</p>
</form>
</body>
</html>